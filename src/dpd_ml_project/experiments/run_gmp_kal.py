"""Driver: run the GMP + Kalman architecture through the generic pipeline.

Run: python -m dpd_ml_project.experiments.run_gmp_kal
"""
...
def run(num_iters: int = 50, seed: int = 1234) -> list[float]:
    """Run ``num_iters`` GMP+Kalman passes; return the EVM (dB) trajectory."""
    np.random.seed(seed)
    sim = SimConfig(signal_rms_dbp=-8.0, snr_db=70.0)
    model, adapter = build_dpd({"arch": "gmp", "adapter": "kal", "r_kal": 1e-6, "q_kal": 1e-6})
